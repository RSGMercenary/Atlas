...
internal class SystemManager : ISystemManager
{
	...
	private TSystem CreateSystem<TSystem>() where TSystem : class, ISystem => Creator?.Create<TSystem>() ?? SystemGetter.GetSystem<TSystem>();
	...
	{
		systems.Remove(system);
		for(var index = systems.Count - 1; index > -1; --index)
		{
			if(systems[index].Priority <= system.Priority)
			{
